...
local function scroll_mouse_window(command)
	vim.api.nvim_win_call(vim.fn.getmousepos().winid, function()
		vim.cmd.normal({ args = { command }, bang = true })
	end)
end
vim.keymap.set('n', '<S-ScrollWheelUp>', function() scroll_mouse_window('3zl') end, { desc = "Scroll right" })
vim.keymap.set('n', '<S-ScrollWheelDown>', function() scroll_mouse_window('3zh') end, { desc = "Scroll left" })
