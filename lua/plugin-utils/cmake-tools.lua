...
local function get_launch_targets(cmake_tools, callback, retried)
	...
	if not config:has_build_directory() then
		cmake_tools.generate({ bang = false, fargs = {} }, function(result)
			if result_ok(result) then
				get_launch_targets(cmake_tools, callback, retried)
			else
				callback(result)
			end
		end)
	...
	local ok, result = pcall(cmake_tools.get_launch_targets)
	if ok then
		callback(result)
		return
	end
	...
	cmake_tools.generate({ bang = false, fargs = {} }, function(generate_result)
		if result_ok(generate_result) then
			get_launch_targets(cmake_tools, callback, true)
		else
			callback(generate_result)
		end
	end)
...
function M.run_single_target(cmake_tools, opts)
	if not cmake_tools.is_cmake_project() then
		vim.notify("Not a CMake project", vim.log.levels.WARN, { title = "CMakeTools" })
		return
	end
	...
	if fargs[1] ~= nil then
		local target = table.remove(fargs, 1)
		run_with_codemodel_retry(cmake_tools, cmake_tools.run, { target = target, args = fargs })
		return
	end

	get_launch_targets(cmake_tools, function(result)
		if not result_ok(result) then
			local message = result and result.message or "Unable to get CMake launch targets"
			vim.notify(message, vim.log.levels.ERROR, { title = "CMakeTools" })
			return
		end

		local targets = result.data and result.data.targets or {}
		local display_targets = result.data and result.data.display_targets or targets
		if #targets == 0 then
			vim.notify("No runnable CMake targets found", vim.log.levels.WARN, { title = "CMakeTools" })
			return
		end

		vim.ui.select(display_targets, { prompt = "Select target to run" }, function(_, idx)
			if not idx then
				return
			end

			run_with_codemodel_retry(cmake_tools, cmake_tools.run, { target = targets[idx], args = {} })
		end)
	end)
end
